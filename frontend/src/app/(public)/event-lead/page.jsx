...
import { getEventLeadLabel } from '@/constants/eventLeads'
...
const PRODUCT_OPTIONS = [
  ...
  { value: 'FR Luvex', label: 'FR Luvex' },
  { value: 'Skystick Adhesive', label: 'Skystick Adhesive' },
]

const USER_TYPE_OPTIONS = ['OEM','Architect/Interior designer', 'Dealer', 'Distributor', 'Contractor', 'End Customer'].map((t) => ({
  value: t,
  label: t,
}))
...
const EventLeadForm = () => {
  ...
  const eventName = state?.eventTitle || getEventLeadLabel(eventPlace)
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      await apiFetch(`/api/lead/event/contact-form-submit/${encodeURIComponent(eventPlace)}`, {
        method: 'POST',
      ...
      navigate('/thank-you', {
        state: {
          message: `Your enquiry for ${eventName} has been submitted successfully. Our team will get in touch with you shortly.`,
          backPath: `/event-lead/${encodeURIComponent(eventPlace)}`,
